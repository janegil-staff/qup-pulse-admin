  ...
  const facts = [
    ...
    profile.gender
      ? {
          key: "gender",
          label: s.gender,
          value: t.app.genders[profile.gender] || profile.gender,
        }
      : null,
    profile.locationName
      ? { key: "locName", label: p.locationName, value: profile.locationName }
      : null,
    profile.language
      ? {
          key: "lang",
      ...
      <main className="mx-auto max-w-2xl px-6 py-8">
        ...
        <div className="mb-4 rounded-2xl border border-slate-300 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#131c26] dark:shadow-none">
            ...
            <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 border-t border-slate-200 pt-4 dark:border-slate-800">
              {facts.map((f) => (
                <div key={f.key}>
                  ...
                  <dd className="mt-0.5 text-[15px] text-slate-800 dark:text-slate-200">
                    {f.value}
                  </dd>
                </div>
              ))}
          ...
          {profile?.bio ? (
            ...
            <p className="mt-5 text-[15px] text-slate-400 dark:text-slate-600">
              {p.noBio}
            </p>
          )}
...
function EditModal({ profile, onClose, onSaved }) {
  ...
  async function onPickFile(e) {
    ...
    try {
      ...
      setPhotos((prev) =>
        [{ url: uploaded.url, publicId: uploaded.publicId }, ...prev].slice(
          0,
          6,
        ),
      );
