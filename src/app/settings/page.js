...
export default function SettingsPage() {
  ...
  function saveAge(which, raw) {
    ...
    if (!Number.isFinite(n) || n < 18 || n > 120) {
      const fb = String(
        profile?.preferences?.[which] ?? (which === "ageMin" ? 18 : 99),
      );
      if (which === "ageMin") setAgeMin(fb);
      else setAgeMax(fb);
      return;
    }
    savePref({ [which]: n });
  ...
  function saveDistance() {
    ...
    if (!Number.isFinite(n) || n < 1) {
      setDistance(
        String(profile?.preferences?.maxDistanceKm ?? DEFAULT_DISTANCE_KM),
      );
      return;
    }
  ...
  const genderLabel = profile?.gender
    ? t.app.genders[profile.gender] || profile.gender
    : "—";
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0b1016] dark:text-slate-100">
      ...
      <main className="mx-auto max-w-2xl px-6 py-8">
        ...
        <Card>
          ...
          <LocationRow
            ...
            value={profile?.locationName || s.notSet}
      ...
      {modal === "location" && (
        <LocationModal onClose={() => setModal(null)} onSaved={reload} />
      )}
      {modal === "delete" && (
        <DeleteAccountModal
          onClose={() => setModal(null)}
...
function ValueRow({ label, value, onClick }) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center justify-between gap-3 border-b border-slate-200 px-5 py-4 text-left text-[15px] transition last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50"
    >
...
function LocationRow({ title, value, onClick, last }) {
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center justify-between gap-3 px-5 py-4 text-left text-[15px] transition hover:bg-slate-50 dark:hover:bg-slate-800/50 ${last ? "" : "border-b border-slate-200 dark:border-slate-800"}`}
    >
...
function ToggleRow({ label, sublabel, value, onChange, last }) {
  return (
    <div
      className={`flex items-center justify-between gap-3 px-5 py-4 ${last ? "" : "border-b border-slate-200 dark:border-slate-800"}`}
    >
      <div>
        <span className="text-[15px] text-slate-900 dark:text-slate-100">
          {label}
        </span>
        {sublabel ? (
...
function ModalButtons({ onClose, onSave, saving, saveLabel, disabled }) {
  ...
  return (
    <div className="mt-5 flex justify-end gap-3">
      <button
        onClick={onClose}
        className="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300"
      >
        {c.cancel}
      </button>
      <button
...
const input =
  "w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-[15px] outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-[#0b1016]";
...
function UsernameModal({ current, onClose, onSaved }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await updateMyProfile({ username: u });
      await onSaved();
      onClose();
    } catch (e) {
      setErr(e.message);
...
function EmailModal({ current, onClose, onSaved }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await updateMyProfile({ email: e, pin });
      await onSaved();
      onClose();
    } catch (er) {
      setErr(er.message);
...
function PinModal({ onClose }) {
  ...
  async function save() {
    if (!/^\d{4}$/.test(cur) || !/^\d{4}$/.test(next))
      return setErr(s.err.bothPins4);
      ...
      {done ? (
        ...
        <>
          ...
          <input
            ...
            value={cur}
            onChange={(e) =>
              setCur(e.target.value.replace(/\D/g, "").slice(0, 4))
            }
            placeholder="••••"
            autoFocus
          />
...
function GenderModal({ current, onClose, onSaved }) {
  ...
  async function pick(g) {
    setSaving(true);
    try {
      await updateMyProfile({ gender: g });
      await onSaved();
      onClose();
    } catch (e) {
      setErr(e.message);
    ...
    >
      ...
      <div className="mt-5 flex justify-end gap-3">
        <button
          onClick={onClose}
          disabled={deleting}
          className="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 disabled:opacity-50 dark:text-slate-300"
        >
          {c.cancel}
        </button>
        <button
...
function LocationModal({ onClose, onSaved }) {
  ...
  async function runSearch() {
    ...
    try {
      ...
      setErr(e.message || s.err.searchFailed);
    } finally {
      setSearching(false);
    }
  }
  ...
  async function pick(place) {
    ...
    try {
      ...
      setErr(e.message || s.err.saveLocation);
      setBusy(false);
    }
  }
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          setErr(e.message || s.err.setLocation);
          setBusy(false);
        }
      },
      () => {
        setBusy(false);
        setErr(s.err.getLocation);
      },
  ...
  return (
    <ModalShell title={s.yourLocation} onClose={onClose}>
      ...
      {err ? (
        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{err}</p>
      ) : null}
