// qup-pulse-admin/src/app/settings/personal/page.js
'use client';

// Personal settings — web version of the app's Personlige innstillinger screen.
// LOGGED-IN ONLY. Fully localized via useLang() (t.app.settings.* / show / genders / common).
// Modals call useLang() themselves since they're defined outside the page component.
//
// Light-mode contrast: cards & modals use border-slate-300 + shadow-sm so they
// read against bg-slate-50. Row dividers use slate-200 (a touch softer than the
// card edge). Dark mode untouched (slate-800 throughout).
...
// discovery "show" state values -> t.app.show.* keys
...
export default function PersonalSettingsPage() {
  ...
  const [distance, setDistance] = useState(null); // null = Anywhere
  ...
  function cycleShow() {
    ...
    const next = SHOW_KEYS[(idx + 1) % SHOW_KEYS.length];
    setShow(next);
    savePref({ show: next });
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0b1016] dark:text-slate-100">
      ...
      <main className="mx-auto max-w-2xl px-6 py-8">
        <h1 className="mb-6 text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{s.title}</h1>
      ...
      {/* Modals */}
      {modal === 'username' && (
        <UsernameModal current={profile?.username || ''} onClose={() => setModal(null)} onSaved={reload} />
      )}
      {modal === 'email' && (
        <EmailModal current={profile?.email || ''} onClose={() => setModal(null)} onSaved={reload} />
      )}
      {modal === 'pin' && (
        <PinModal onClose={() => setModal(null)} />
      )}
      {modal === 'gender' && (
        <GenderModal current={profile?.gender} onClose={() => setModal(null)} onSaved={reload} />
      )}
      {modal === 'language' && (
        <LanguageModal current={lang} setLang={setLang} onClose={() => setModal(null)} />
      )}
      {modal === 'location' && (
        ...
        {GENDER_KEYS.map((g) => (
          <button key={g} onClick={() => pick(g)} disabled={saving}
            className={`rounded-xl border px-4 py-3 text-sm font-medium transition ${
              current === g ? 'border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'border-slate-300 hover:border-slate-400 dark:border-slate-700'
            }`}>
            {t.app.genders[g]}
          </button>
        ))}
