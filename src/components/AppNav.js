...
export default function AppNav() {
  ...
  // COMBINED_UNREAD_NAV_V2 — the badge counts inbox unreads AND incoming
  // message requests as one number. /messages shows Inbox and Requests as
  // sibling tabs, so a single count resolves either way.
  //
  // Read `total`, NOT `count`. The server scopes `count` to ACCEPTED
  // conversations and returns pending ones separately as `requestCount`, so
  // rendering `count` alone meant an incoming request never moved the badge —
  // and a request with no messages yet contributed nothing anywhere. V1 of
  // this comment claimed the server pre-combined them; it does not, and
  // chatUnreadCount() now does the addition and exposes it as `total`.
  //
  // Fed by chat:notify, which deliver() emits to `user:${id}` for every
  // participant who isn't the sender — so it fires regardless of which tab
  // you're on, unlike chat:message, which only reaches sockets that have
  // joined a convo: room.
  //
  ...
  return (
    ...
    when scanning several windows to find the right session. */}
        <div className="flex min-w-0 items-center gap-3">
          ...
          >
            <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" /> Qup
            Pulse
          </Link>
          {me ? (
            <span
...
function TabLink({ tab, n, m, active, block, unread = 0, onNavigate }) {
  ...
  return (
    <Link
      ...
      className={`${base} relative flex items-center gap-2 border ${
        active
